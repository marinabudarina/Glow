...
const CHAR_MS = 62;       // ms per character
const HOLD_AFTER_MS = 900; // pause once fully typed before transitioning
...
// ─── WebGL glow canvas ───────────────────────────────────────────────────────
function GlowCanvas({ visible }: { visible: boolean }) {
  const hostRef = useRef<HTMLDivElement>(null);
  const engineRef = useRef<BlurGlow | null>(null);

  useEffect(() => {
    if (!hostRef.current) return;
    const engine = new BlurGlow(hostRef.current, "Inter");
